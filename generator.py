...
class AudioProcessor(object):
  ...
  def prepare_processing_graph(self, model_settings):
    ...
    print("wav:",self.wav_filename_placeholder_)
    
    wav_loader = tf.io.read_file(self.wav_filename_placeholder_)
    print("wav_loader", wav_loader)
    wav_decoder = tf.audio.decode_wav(
        wav_loader, desired_channels=1, desired_samples=desired_samples)
    print("wav_decoder:", wav_decoder)
    ...
    print('spectrogram:', self.spectrogram_)
    print('spectrogram:', self.spectrogram_.shape[-1])
    ...
    """self.mfcc_ = mel_fbanks.mel_filter_banks(nfilts=48,
                                              nfft=512,
                                              fs=16000,
                                              low_freq=0,
                                              high_freq=8000,
                                              scale="constant")[:, :
                              model_settings['num_log_mel_features']]  # :13"""
    #self.mfcc_ = plp(background_add, 16000, 13)
    print("mfcc:",self.mfcc_.shape)
